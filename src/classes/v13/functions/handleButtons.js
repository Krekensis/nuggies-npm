...
const defaultGiveawayMessages = {
  ...
  endedGiveawayDescription:
    "🎁 Prize: **{prize}**\n🎊 Hosted by: {hostedBy}\n⏲️ Winner(s): {winners}",
  giveawayFooterImage:
    "https://cdn.discordapp.com/emojis/843076397345144863.png",
...
module.exports = async (client, button) => {
  ...
  if (id.startsWith("giveaways")) {
    ...
    if (tag[1] === "enter") {
      ...
      if (data.requirements.enabled) {
        let amaridata = null;
        if (data.requirements.amariweekly || data.requirements.amarilevel)
          amaridata = await utils.getAmariData(
            data.requirements.key,
            button.user.id,
            button.guild.id
          );
        if (data.requirements.roles) {
          const roles = data.requirements.roles.map((x) =>
            button.message.guild.members.cache
              .get(button.user.id)
              .roles.cache.get(x)
          );
          if (
            ...
            !button.message.guild.members.cache
              .get(button.user.id)
              .roles.cache.some((e) => e.id == "858901404507963403") &&
            !button.message.guild.members.cache
              .get(button.user.id)
              .roles.cache.some((e) => e.id == "853152861746364448") &&
            !button.message.guild.members.cache
              .get(button.user.id)
              .roles.cache.some((e) => e.id == "953949479227232326")
          ) {
            const requiredRoles = button.message.guild.roles.cache
              ...
              .join(", ");
            return button.reply({
              content: client.customMessages.giveawayMessages.nonoRole.replace(
                /{requiredRoles}/g,
                requiredRoles
              ),
              ephemeral: true,
        ...
        if (data.requirements.amariweekly) {
          if (
            ...
            return button.reply({
              content: "There was an error fetching your amari data",
              ephemeral: true,
            });
          if (
            parseInt(data.requirements.amariweekly) >
            parseInt(amaridata.weeklyExp)
          ) {
            return button.reply({
              content: client.customMessages.giveawayMessages.noWeeklyExp,
              ephemeral: true,
            });
          }
        }
        if (data.requirements.amarilevel) {
          ...
          if (parseInt(data.requirements.level) > amaridata.level) {
            return button.reply({
              content: client.customMessages.giveawayMessages.noLevel,
              ephemeral: true,
            });
          }
        }
      ...
      if (!data.clickers.includes(button.user.id)) {
        ...
        data.save();
        return button.reply({
          content: client.customMessages.giveawayMessages.newParticipant
            .replace(/{winPercentage}/g, (1 / data.clickers.length) * 100)
            .replace(/{totalParticipants}/g, data.clickers.length),
          ephemeral: true,
        });
      } else {
        return button.reply({
          content: client.customMessages.giveawayMessages.alreadyParticipated,
          ephemeral: true,
        });
      }
    }
    if (tag[1] === "reroll") {
      if (button.user.id !== tag[2])
        return button.reply({
          ephemeral: true,
          content: "You Cannot Reroll This Giveaway, Only The Host Can",
        });
      try {
        ...
        return button.message.channel.send(
          "⚠️ **Unable To Find That Giveaway**"
        );
      }
      if (!win.length)
        return button.message.channel.send(
          client.customMessages.giveawayMessages.nonoParticipants
        );
      button.message.channel.send({
        content: client.customMessages.giveawayMessages.rerolledMessage.replace(
          /{winner}/g,
          `<@${win}>`
        ),
        components: [
          new Discord.MessageActionRow().addComponents([
            new Discord.MessageButton()
              .setLabel("Giveaway")
              .setURL(
                `https://discord.com/channels/${button.message.guild.id}/${button.message.channel.id}/${button.message.id}`
              )
              .setStyle("LINK"),
          ]),
        ],
  ...
  if (id.startsWith("br")) {
    ...
    if (button.member.roles.cache.has(role)) {
      ...
      button.reply({
        content:
          client.customMessages.buttonRolesMessages.removeMessage.replace(
            /{role}/g,
            `<@&${role}>`
          ),
        ephemeral: true,
      ...
      button.reply({
        content: client.customMessages.buttonRolesMessages.addMessage.replace(
          /{role}/g,
          `<@&${role}>`
        ),
        ephemeral: true,
